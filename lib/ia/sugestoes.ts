...
export async function sugerirAlternativas(d: { original: Exercicio; limitacoes: string[]; candidatos: Exercicio[]; observacao?: string | null }): Promise<Sugestao[]> {
  ...
  const lista = d.candidatos.slice(0, 25).map((c) => ({ id: c.id, nome: c.nome, equipamento: c.equipamento, mecanica: c.mecanica, nivel: c.nivel, contraindicacoes: c.contraindicacoes }));
  let resp;
  try {
    ...
    console.error("IA sugestões falhou:", e instanceof Error ? e.message : e);
    return [];
  }
